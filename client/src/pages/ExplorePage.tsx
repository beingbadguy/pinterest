...
const ExplorePage = () => {
  const { allUsers } = usePinterestStore();
  ...
  return (
    <div>
      <div className="border flex items-center gap-1 rounded  border-gray-400 ">
        <input
          type="text"
      ...
      <h1 className="font-semibold text-xl mt-1">All Users</h1>
      <div>
        {findUser.length > 0 ? (
                ...
                <div
                  className={`${
                    onlineUsers && onlineUsers.includes(user._id)
                      ? ""
                      : "hidden"
                  } absolute size-4 bg-green-500 rounded-full left-10 top-2`}
                ></div>
                <img
                  src={user.profilePic}
                  alt={user.username}
